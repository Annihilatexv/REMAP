...
# Adapted for our model evaluation.
...
def transform_dynamic_skip(model: MixtralForCausalLM, experts_difference: list = None):
    ...
    if experts_difference is None:
        experts_difference = [None] * len(model.model.layers)
...
def load_model_tokenizer(
    ...
    # Evaluate semi-structured models
    sparsity_type = getattr(model.config, 'sparsity_type', getattr(model.config, 'wanda_sparsity_type', None))
    if sparsity_type and ':' in sparsity_type:
        logger.info(
            f'Using semi-structured model ({sparsity_type}). Now transforming to sparse semi-structured linear layers...')
        with torch.no_grad():
...
def main():
    ...
    parser.add_argument("--model_name_or_path", type=str,
                        default="path/to/Mixtral-8X7B-v0.1")
